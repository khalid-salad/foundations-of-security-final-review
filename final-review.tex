...
\begin{itemize}[nosep]
    ...
    \item More secure
          ...
          \begin{figure}[H]
              \begin{tikzpicture}
                  \node[draw, circle] (internet) {Internet};
                  \node[draw, rectangle, right =of internet] (fw1) {firewall};
                  \node[draw, circle, right=of fw1] (dmz) {DMZ};
                  \node[draw, rectangle, right =of dmz] (fw2) {firewall};
                  \node[align=center,draw, circle, right=of fw2] (network) {internal\\network};
...
\begin{itemize}[nosep]
    \item \mintinline{c}|int chroot(const char *path)|
...
\begin{itemize}[nosep]
    \item \mintinline{c}|int seccomp(SECCOMP_SET_MODE_STRICT, 0, NULL)|
    ...
    \item Once a process enters seccomp mode, it cannot make any system calls, except for
          \begin{itemize}[nosep]
              \item \mintinline{c}|exit(...)|, \mintinline{c}|sigreturn(...)|
              \item \mintinline{c}|read(int filedescriptor, ...)|
              \item \mintinline{c}|write(int filedescriptor, ...)|
          \end{itemize}
              ...
              \item approaches
                    \begin{itemize}[nosep]
                        ...
                        \item Network Address Translation (NAT)
                              \begin{itemize}[nosep]
                                  \item address translation has an inherent “source validation side effect”
                              \end{itemize}
                        \item Reverse Path Forwarding
                              \begin{itemize}[nosep]
                                  \item forwarding decision for a packet is affected by its source address
                                  \item used for loop-free forwarding of multicast packets and for ingress filtering
                              \end{itemize}
                        \item Unicast Reverse Path Forwarding (uRPF)
                              \begin{itemize}[nosep]
                                  \item strict mode: test if the incoming interface is the best path to the source according to forwarding information base (FIB), a.k.a, forwarding table $\rightarrow$ drop failed packets
                                  \item Problem: routing is often asymmetric on the Internet backbone
                                  \item strict mode would filter out packets on asymmetric routes
                                  \item feasible mode: maintain multiple feasible paths in the FIB
                                  \item loose mode: verify the existence of a route (regardless of interface of direction)
                              \end{itemize}
                    \end{itemize}
          \end{itemize}
    \item CAPTCHA (Completely Automated Public Turing test to tell Computers and Humans Apart)
          \begin{itemize}[nosep]
              \item challenge-response test used to determine if a client is human
              \item typically based on hard AI problems, such as image recognition (e.g., character recognition)
